...
def get_report_44(buffer, document, currency, document_type, request):
    ...
    pdf.setFont('Helvetica-Bold', 20)
    pdf.setFillColor(colors.green)
    pdf = draw_wrapped_line(pdf, document["customer"]["business_name"].title(), 100, 10, 120, 10)
    pdf.setFillColor(colors.black)
    # pdf.drawRightString(width - 55, 20, f"{document_type.upper()}")
    pdf.setFont('Helvetica', 10)
    pdf = draw_wrapped_line(pdf, document["customer"]["address"].capitalize(), 100, 10, 140, 10)
    pdf = draw_wrapped_line(pdf, document["customer"]["email"], 100, 10, 155, 10)
    pdf = draw_wrapped_line(pdf, document["customer"]["phone_number"], 100, 10, 170, 10)
