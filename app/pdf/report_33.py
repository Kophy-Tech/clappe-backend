...
def add_another_page(pdf, item_list, currency, document, document_type):
    ...
    fill_colour = colors.Color(165/255, 5/255, 5/255)


    pdf.setFillColor(fill_colour)
    pdf.setStrokeColor(fill_colour)
    pdf.line(10, 10, width-55, 10)
    pdf.drawString(10, 25, "Qty")
...
def total_box(pdf, start_y, currency, document_type, document):
    ...
    if document_type == "invoice":
        ...
        pdf.drawString(10, start_y+120, "Total")
        ...
        pdf.setFillColor(fill_colour)
        pdf.setFont('Helvetica', 23)
        pdf.drawString(10, start_y+100, "Total")
...
def get_report_33(buffer, document, currency, document_type, request, logo):
    ...
    pdf.drawImage("app/pdf/logo_33.png", -30, -30, width=350, height=195)
            
    fill_colour = colors.Color(165/255, 5/255, 5/255)

    pdf.setFont('Helvetica', 10)
    pdf = draw_wrapped_line(pdf, request.user.business_name.title(), 100, 10, 200, 10)
    pdf = draw_wrapped_line(pdf, request.user.address.capitalize(), 40, 10, 210, 10)
    pdf = draw_wrapped_line(pdf, request.user.email, 100, 10, 235, 10)
    pdf = draw_wrapped_line(pdf, request.user.phone_number, 100, 10, 245, 10)
    ...
    pdf.setFont('Helvetica', 10)
    pdf.drawString(10, 180, "From")
    pdf.drawString(190, 180, "Bill To")
    pdf.drawString(360, 180, "Ship To")
    pdf.setFillColor(colors.black)
    pdf.setFont('Helvetica', 10)
    pdf = draw_wrapped_line(pdf, document["bill_to"], 40, 190, 200, 10)
    pdf = draw_wrapped_line(pdf, document["ship_to"], 40, 360, 200, 10)

    pdf.setFont('Helvetica-Bold', 15)
    pdf.setFillColor(fill_colour)
    pdf.drawRightString(width-55, 17, f"{document_type.upper()}")

    pdf.setFont('Helvetica-Bold', 10)
    pdf.drawString(360, 50, f"{document_type.title()} #")
    pdf.drawString(360, 70, f"{document_type.title()} Date")
    pdf.drawString(360, 90, "Due Date")
    pdf.setFillColor(colors.black)
    pdf.setFont('Helvetica', 10)
    ...
    pdf.drawRightString(width - 55, 50, f"{document[doc_number_key]}")
    pdf.drawRightString(width - 55, 70, f"{document[doc_date_key]}")
    pdf.drawRightString(width - 55, 90, f"{document['due_date']}")
    ...
    pdf.line(8, 255, width-55, 255)
    pdf.drawString(10, 275, "Qty")
    pdf.drawString(150, 275, "Description")
    pdf.drawRightString(400, 275, "Unit Price")
    pdf.drawRightString(width-55, 275, "Amount")
    pdf.line(8, 285, width-55, 285)
