...
def add_another_page(pdf, item_list, currency, document, document_type):
    ...
    pdf.drawImage("app/pdf/logo_6_up.png", -30, -30, width=600, height=500)

    fill_colour = colors.Color(47/255, 128/255, 195/255)
...
def total_box(pdf, start_y, currency, document_type, document):
    fill_colour = colors.Color(47/255, 128/255, 195/255)
    pdf.drawImage("app/pdf/logo_6_down.png", -30, 530, width=600, height=290,showBoundary=False)
...
def get_report_6(buffer, document, currency, document_type, request, logo):
    ...
    pdf.setFillColor(colors.black)
    pdf.setFont('Helvetica', 10)
    pdf = draw_wrapped_line(pdf, request.user.address.capitalize(), 100, 10, 85, 10)
    pdf = draw_wrapped_line(pdf, request.user.email, 100, 10, 105, 10)
    pdf = draw_wrapped_line(pdf, request.user.phone_number, 100, 10, 120, 10)
