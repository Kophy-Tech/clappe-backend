...
def add_another_page(pdf, item_list, currency, document, document_type):
    ...
    pdf.drawImage("app/pdf/logo_14_table.png", -30, -30, width=width, height=120)
...
def total_box(pdf, start_y, currency, document_type, document):
    fill_colour = colors.Color(52/255, 101/255, 164/255)
    width = pdf._pagesize[0]
    pdf.drawImage("app/pdf/logo_14_down.png", -35, 595, width=width+10, height=220)
...
def get_report_14(buffer, document, currency, document_type, request):
    ...
    pdf.drawImage("app/pdf/logo_14_up.png", -30, -30, width=width, height=400)
            


    pdf.setFont('Helvetica-Bold', 20)
    fill_colour = colors.Color(62/255, 109/255, 167/255)
    pdf.setFillColor(fill_colour)
    ...
    pdf.setFont('Helvetica', 10)
    pdf = draw_wrapped_line(pdf, document["bill_to"], 25, 30, 210, 10)
    pdf = draw_wrapped_line(pdf, document["ship_to"], 40, 160, 210, 10)
    ...
    pdf.drawRightString(width - 85, 190, f"{document[doc_number_key]}")
    pdf.drawRightString(width - 85, 210, f"{document[doc_date_key]}")
    pdf.drawRightString(width - 85, 230, f"{document['due_date']}")
