...
def draw_invoice_details(pdf, document_type, document_number):
    fill_color = colors.Color(0, 0, 0, 0.5)
    pdf.saveState()
    pdf.rotate(270)
...
def add_another_page(pdf, item_list, currency, document, document_type):
    ...
    fill_color = colors.Color(0, 0, 0, 0.04)
    ...
    if item_len <= 20:
        ...
        for item in item_list:
            ...
            if item != item_list[-1]:
                print(item)
                pdf.line(40, start_y+10, 540, start_y+10)
...
def get_report_20(buffer, document, currency, document_type, request, logo):
